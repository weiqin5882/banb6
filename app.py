...
def build_summary(
    ...
    official_summary_df = (
        official_clean[official_clean["status"].isin(summary_status)].copy()
        if not official_clean.empty
        else official_clean
    )

    total_sales = float(official_summary_df["sales_amount"].sum()) if not official_summary_df.empty else 0.0
    total_cost = float(official_summary_df["cost_amount"].sum()) if not official_summary_df.empty else 0.0
    total_profit = round(total_sales - total_cost, 2)

    matched_count = int((result_df["比对结果"] == "匹配").sum()) if not result_df.empty else 0
    summary_count = int(len(official_summary_df)) if not official_summary_df.empty else 0
    ...
    loss_count = int((official_summary_df["sales_amount"] - official_summary_df["cost_amount"] < 0).sum()) if not official_summary_df.empty else 0
...
def compare_api():
    try:
        ...
        summary = build_summary(result_df, official_clean, official_stats, service_stats)
